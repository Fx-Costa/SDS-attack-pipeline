...
class Navigator ():
    ...
    def __init__(self, config):
        ...
        self.template_original_loc = os.path.join(
            '../../..', 'template', 'data_showcase.pbit')
        ...
        self.data_schema_loc = os.path.join(
            self.temporary_folder_loc, 'DataModelSchema')
        self.data_mashup_loc = os.path.join(
            self.temporary_folder_loc, 'DataMashup')
        self.data_mashup_zip_loc = os.path.join(
            self.temporary_folder_loc, 'zip_archive.zip')
        self.data_mashup_temp_loc = os.path.join(
            self.temporary_folder_loc, 'zip_archive')
        self.m_file_loc = os.path.join(
            self.data_mashup_temp_loc, 'Formulas', 'Section1.m')
        self.layout_loc = os.path.join(
            self.temporary_folder_loc, 'Report', 'Layout')
        self.template_final_loc = os.path.join(
            self.output_dir, f"{self.prefix}_data_showcase.pbit")
        self.max_number_of_visuals = 64
        self.number_of_visuals_per_page = 16
        self.number_of_combo_tables = 10
        self.max_number_of_pages = math.ceil(
            self.max_number_of_visuals/self.number_of_visuals_per_page)
    ...
    def estimated_measure(self):
        ...
        whole = target_attribute + filtered_attribute + \
            id_table + estimated_count + '\nRETURN estimated_count'
        return whole
    ...
    def change_table_schema(self, data, visual_name, table_name):
        ...
        expression_copy = copy.deepcopy(
            data['model']['tables'][table_index]['partitions'][0]['source']['expression'])
        expression_updated = self.add_filter_steps(
            expression_copy, filters_list)
    ...
    def change_visual(self, attr_container, box, name, title, combo_table=None, logs=False):
        ...
        default_table = combo_table if (
            combo_table and not logs) else 'synthesized_pivoted'
        ...
        visual_config['singleVisual']['projections']['Values'][0][
            'queryRef'] = f'CountNonNull({default_table}.{event})'
        visual_config['singleVisual']['projections'][
            'Category'][0]['queryRef'] = f'{table}.{name}'
        ...
        visual_config['singleVisual']['prototypeQuery']['Select'][
            1]['Name'] = f'CountNonNull({default_table}.{event})'
        ...
        visual_config['singleVisual']['prototypeQuery'][
            'Select'][0]['Name'] = f'{table}.{name}'
        ...
        if self.event_column and name == self.event_column:
            visual_config['singleVisual']['projections']['Color'] = [
    ...
    def calculate_visual_boxes(self, prepared_layout):
        ...
        for page in prepared_layout:
            ...
            for i in range(len(page)):
                ...
                y = y_start + margin * \
                    (math.floor(i/page_layout[1])) + \
                    height*(math.floor(i/page_layout[1]))
    ...
    def process(self):
        ...
        if len(df) == 0:
            ...
            for i, row in df.iterrows():
                [new_df.append([i, ind, value])
                 for ind, value in row.items() if str(value) != '']
            ...
            logging.info(
                f'Done with record files in {time.time() - start_time} seconds')
            ...
            if self.template_combined_attributes:
                checked_type = isinstance(
                    self.template_combined_attributes, dict)
                if not checked_type:
                    logging.info(
                ...
                combo_tables = {each: f'copy_{i+1}' for i,
                                each in enumerate(combo_list)}
            ...
            data['model']['tables'][4]['measures'][1]['expression'] = self.actual_measure(
                combo_tables)
            ...
            if combo_tables:
                for visual_name, table_name in combo_tables.items():
                    data = self.change_table_schema(
                        data, visual_name, table_name)
            ...
            if combo_tables:
                ...
                with open(self.m_file_loc, 'r', encoding='utf-8') as myfile:
                    mashup = myfile.read()
                for visual_name, table_name in combo_tables.items():
                    mashup = self.change_table_mashup(
                        mashup, visual_name, table_name)
                with open(self.m_file_loc, 'w', encoding='utf-8') as outfile:
                    outfile.writelines(mashup)

                # zip the archive with changes
                make_archive(self.data_mashup_temp_loc,
                             'zip', self.data_mashup_temp_loc)
                with open(self.data_mashup_zip_loc, "rb") as z:
                    zip_arch = z.read()
                size_of_zip_bn = (len(zip_arch)).to_bytes(
                    4, byteorder='little')
                ...
                logging.info(
                    'Tables are filtered to support combined attributes')
            ...
            attributes_viz_index = [2, 7, 10, 13, 5, 8,
                                    11, 14, 6, 9, 12, 15, 16, 17, 18, 19]

            for i, page in enumerate(pages):
                names_index = 0
                page['displayName'] = page_names[i] if len(
                    page_names) > i else page['displayName']
                ...
                if self.template_title:
                    ...
                    new += [containers[ind]
                            for ind in persistent_viz_index[2:]]
                else:
                ...
                while viz_index < len(attributes_viz_index) and names_index < len(prepared_layout[i]):
                    attr_container = copy.deepcopy(
                        containers[attributes_viz_index[viz_index]])
                    ...
                    if name in combo_tables:
                        ...
                        self.change_visual(
                            attr_container, bounding_boxes[i][names_index], name, name)
            ...
            make_archive(self.temporary_folder_loc,
                         'zip', self.temporary_folder_loc)
